#!/usr/bin/env bash
# macOS-specific build script for GRASS GIS
set -uxo pipefail
...
CONFIG_FLAGS=(
  ...
  --with-lapack
  --with-readline
  --without-openmp  # OpenMP support may differ on macOS
)
...
# Clean and build
echo "=== Starting GRASS build for macOS ===" >&2
make distclean || true
...
if [ -f include/Make/Platform.make ]; then
    ...
    # macOS uses different linker flags (no --no-as-needed)
    sed -i.bak 's/^ICONVLIB[[:space:]]*=.*/ICONVLIB = -liconv/' include/Make/Platform.make
    echo "=== After fix ===" >&2
